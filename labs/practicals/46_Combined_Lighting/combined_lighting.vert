...
void main() {
  ...
  // Calculate k
  float diffuseK = max(dot(transformed_normal, light_dir), 0.0);
  // Calculate diffuse
  vec4 diffuse = diffuseK * (diffuse_reflection * light_colour);
  ...
  vec3 half_vector = normalize(light_dir + viewDir);

  // Calculate k
  float k = pow(max(dot(transformed_normal, half_vector), 0.0), shininess);

  // Calculate specular
  vec4 specular = k * (light_colour * specular_reflection);

  // Output combined components
  vertex_colour = diffuse + ambient + specular;
  // *********************************
  // Ensure alpha is 1
  vertex_colour.a = 1.0;
}